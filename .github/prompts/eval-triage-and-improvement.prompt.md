...
This prompt serves **Stages 2-4** of the [MS Learn 4-stage evaluation framework](https://learn.microsoft.com/en-us/microsoft-copilot-studio/guidance/evaluation-checklist) — the iterative loop of running evals, diagnosing failures, applying fixes, and re-running. In Stage 4 (Operationalize), this prompt helps triage regressions caught by CI/CD eval runs after agent updates. Use the [evaluation checklist template](https://github.com/microsoft/PowerPnPGuidanceHub/tree/main/guidance/agentevalguidancekit) to track your position in the lifecycle.
...
This triage report accelerates diagnosis but does not replace human judgment. Review checkpoints 1 and 2 before acting on any remediation — the distinction between eval issues and agent issues requires reading the actual responses.
...
This prompt works alongside the **AI Agent Evaluation Scenario Library** (`github.com/microsoft/ai-agent-eval-scenario-library`), which defines the scenarios and quality signals that produce the eval results this triage prompt helps interpret, and the **Triage & Improvement Playbook** (`github.com/microsoft/triage-and-improvement-playbook`), which provides the diagnostic frameworks used in this prompt's triage steps.
