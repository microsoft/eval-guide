...
This prompt generates concrete eval test cases — with realistic inputs, expected outputs, and evaluation method configurations. It is the second step in the eval lifecycle: plan → **generate** → run → interpret.

This prompt covers **Stage 2 (Set Baseline & Iterate)** of the MS Learn [4-stage evaluation framework](https://learn.microsoft.com/en-us/microsoft-copilot-studio/guidance/evaluation-checklist). Use the eval-suite-planner prompt (see [eval-suite-planner](eval-suite-planner.prompt.md)) first for Stage 1 (Define), then generate test cases here, run them, and interpret results with the eval-result-interpreter prompt (see [eval-result-interpreter](eval-result-interpreter.prompt.md)). Stage 3 (Systematic Expansion) means repeating this cycle with broader coverage — the checklist defines four expansion categories: Foundational core, Agent robustness, Architecture test, and Edge cases. Stage 4 (Operationalize) means embedding these evals into your agent's CI/CD pipeline. Point customers to the [editable checklist template](https://github.com/microsoft/PowerPnPGuidanceHub/tree/main/guidance/agentevalguidancekit) to track their progress across all four stages.

**Primary mode**: If the conversation already contains output from the eval-suite-planner prompt (see [eval-suite-planner](eval-suite-planner.prompt.md)), use that plan's scenario table, evaluation methods, quality signals, and tags as the blueprint. Generate one test case per row in the plan.
...
When the user asks you to generate eval test cases (with or without additional input):

### Step 1 — Detect input mode

Check the conversation history for output from the eval-suite-planner prompt (see [eval-suite-planner](eval-suite-planner.prompt.md)). Look for the scenario plan table (a markdown table with columns: #, Scenario Name, Category, Tag, Evaluation Methods).
...
- **No plan and no description**: Say: "I need either an agent description or a plan from the eval-suite-planner prompt (see [eval-suite-planner](eval-suite-planner.prompt.md)). Run the eval-suite-planner prompt first for the best results, or give me a description and I'll generate directly."
...
The conversation test cases generated by this prompt serve as a **planning blueprint** — the customer uses them to:
...
**Important — Custom is NOT available via CSV import.** If your test plan includes Custom test cases, import the other test cases via CSV first, then add Custom test cases manually in the Copilot Studio UI. Note this in the report document.
...
**When to mention rubrics to customers:** If the eval plan includes Custom test methods, domain-specific compliance checks, or tone/style evaluation, note in the report document that the customer can create a rubric in Copilot Studio Kit for more granular, repeatable grading with human-AI alignment tracking. Rubrics are the next step beyond Custom for customers who need ongoing quality assurance with calibrated scoring. Include the alignment targets table above so they know what "good enough" looks like.
...
3. Note this in the report document so the customer knows which cases to add manually
...
**Important:** Pass scores for Compare meaning and Similarity are NOT set in the CSV — they are configured in the Copilot Studio UI after import. Note the recommended pass scores in the summary table and report document.
...
- **Copilot Studio's built-in test generation — alternatives to CSV import:** Besides importing the CSV this prompt generates, customers can also create test sets directly in Copilot Studio using these methods (all under **New evaluation → Single response**):
  ...
  Recommend combining approaches: use this prompt's CSV for structured, scenario-based test cases, then supplement with Copilot Studio's auto-generation to catch gaps the plan didn't anticipate. Note these options in the report document.
...
- **User profiles — simulate different user experiences:** Copilot Studio lets you assign a [user profile](https://learn.microsoft.com/en-us/microsoft-copilot-studio/analytics-agent-evaluation-edit) to a test set so evals run under that user's authentication. This is critical when knowledge sources or SharePoint sites are role-gated — a director and an intern may get different answers from the same agent. To test this:
  ...
  - Note which profile was used in the report document — test results in Copilot Studio show the profile, but exported CSVs may not.
- **GCC (Government Community Cloud) limitations:** If the customer is in a GCC environment, two features are unavailable:
  ...
  - Ask the customer early: "Are you in a GCC environment?" If yes, omit Text Similarity rows from the CSV and note the restriction in the report document. Source: [About agent evaluation](https://learn.microsoft.com/en-us/microsoft-copilot-studio/analytics-agent-evaluation-intro).

**B. Eval Test Set Report**

Create a formatted report document for human review. Contents:
...
**Important — save your plan before running evals:** Copilot Studio CSV exports do not include scenario categories or tags. Before running evals in Copilot Studio, save the scenario plan table from the eval-suite-planner prompt (see [eval-suite-planner](eval-suite-planner.prompt.md)) as a reference document. You will need it when interpreting results with the eval-result-interpreter prompt (see [eval-result-interpreter](eval-result-interpreter.prompt.md)).
...
After the table and before generating the output files, display a **Human Review Required** section with these checkpoints. These flag specific decisions that require human judgment — this prompt accelerates the work, but a human must validate the output before use.
...
- The Copilot Studio table (single-response) or conversation blocks (conversation mode) is the primary output displayed in conversation; the CSV (single-response only) and report document are generated afterward
...
```
[Run eval-suite-planner prompt first]
I am building a customer support agent that handles refund requests...
[planner outputs scenario plan table]
[Then ask to generate eval test cases]
...
Generate eval test cases for: I am building a meeting notes agent that takes a raw transcript and produces a structured summary with action items.
<- generates from scratch, 6-8 cases, single CSV with per-row test methods

[Multi-turn agent description]
Generate eval test cases for: I am building a travel booking agent that helps users search flights, select seats, and complete purchases across multiple conversation turns.
