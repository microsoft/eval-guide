...
This prompt takes a plain-English description of an agent and produces a structured eval suite plan. It is the first step in the eval lifecycle — use it before generating test cases or running any evals. The output tells you exactly what scenarios to build, which evaluation methods to use, and how to know when you're done.

This prompt covers **Stage 1 (Define)** of the MS Learn 4-stage evaluation framework. After planning, use the eval-generator prompt (see [eval-generator](eval-generator.prompt.md)) for Stage 2 (Set Baseline & Iterate), then expand coverage (Stage 3) and operationalize into CI/CD (Stage 4).

**Knowledge sources:** This prompt's guidance is grounded in three Microsoft sources:
...
When the user asks you to plan an eval suite for their agent, read the description, infer the agent's primary task, key capabilities, and failure modes, then produce the following output in this exact order. Do not ask clarifying questions, do not pad responses, do not hedge.
...
- Synthetic test cases (from the eval-generator prompt (see [eval-generator](eval-generator.prompt.md))) = structured coverage of planned scenarios, edge cases, and adversarial tests
...
|---|---|---|
| **CSV import** (from the eval-generator prompt (see [eval-generator](eval-generator.prompt.md))) | Import structured test cases with Question, Expected response, and Testing method columns | Always — this is the primary method for structured, scenario-driven coverage |
...
This table is the primary handoff artifact to the eval-generator prompt (see [eval-generator](eval-generator.prompt.md)) — the generator will produce one test case per row. Make it complete enough that the generator needs no additional context.
...
- **Alignment target:** 75-90% average alignment (formula: `100% x (1 - |AI grade - Human grade| / 4)`). Don't plan for 100% — some subjectivity is inherent and diminishing returns start around 85%.
...
**Data retention:** Copilot Studio retains test run results for **89 days only** — after that, results are permanently deleted. Plan an export habit from run #1:
...
**A. Eval Suite Plan Report**
Create a formatted report document containing:
...
- **Mandatory reminder:** "This eval plan was AI-generated based on your agent description. Before proceeding to test case generation with the eval-generator prompt (see [eval-generator](eval-generator.prompt.md)), review the scenarios, thresholds, and priority order with your team. The plan should reflect your actual business requirements, not just best-practice defaults."
...
```
I am building a customer support agent that handles refund requests. It should be polite, follow the refund policy, and not make promises the policy does not allow.

I am building a RAG agent that answers questions about our internal HR policy documents. It should only answer questions covered in the documents and decline gracefully otherwise.

I am building an email triage agent that reads incoming emails and labels them urgent, not-urgent, or spam. It should never label a real customer email as spam.

I am building a code review agent that reviews Python pull requests and flags potential bugs, style violations, and missing tests.
```
